...
const Menu = () => {
  ...
  return (
    <div className='px-4 py-22 max-w-6xl mx-auto'>
      {/* section header */}
      ...
      <div className='text-center mb-8'>
        <h2 className='text-2xl font-medium mb-4 text-gray-700'>
          Explore our diverse categories to find your favorite dishes!
        </h2>
        <ul className='flex flex-wrap justify-center gap-4'>
          {categoryItem.map((item, index)=> (
            <li key={index} onClick={()=> setCategory((prev)=>prev === item.category_title ? "All" : item.category_title)}
            className={`cursor-pointer px-6 py-2 rounded-full font-medium text-am transition-all duration-200 ${category === item.category_title ? "bg-amber-500" : "bg-gray-200  hover:bg-amber-100"}`}>
              {item.category_title}
            </li>
          ))
          }
          ...
          products.length > 0 ? (
            products.filter((product)=> category === "All" || category === product.category).map((product)=> 
              <div key={product._id} className='flex items-center gap-6 p-4 border border-gray-100 rounded-xl shadow-sm transition hover:shadow-md'>
                ...
                <p className='text-sm text-gray-500 mt-1'>Delicious Food</p>
            ...
            <p className=' text-sm col-span-2 text-gray-500'>No products available in this category.</p>
          )
